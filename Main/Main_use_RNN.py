...
#uwnd = util.convert_list(df, 'UWND') #바람 제거
sst_diff = util.convert_list(df, 'PAC_IND_SST_DIFF')


#2차원 데이터로 구축 ==> RNN을 위해 3차원 (시간 추가)
for i in range(m):
    x1=[]
    x1.append(eq_soi[i])
#    x1.append(uwnd[i])
...
#Train 1 = 대한해협, Train 2 = 남해
model = Sequential()

# ## 은닉층을 정의함. 유닛개수 100개, input_shape : 샘플수를 제외한 입력형태를 정의함.
# model.add(SimpleRNN(32, input_shape=(2,5692)))
...
##############################LSTM#######################
## 은닉층을 정의함. 유닛개수 100개, input_shape : 샘플수를 제외한 입력형태를 정의함. LSTM 모델
model.add(LSTM(32, input_shape=(2,5692)))
...
loss_1, accuracy_KS = model.evaluate(x_val, y1_val_encoded, verbose=0)
print(accuracy_KS)
loss_2, accuracy_SS = model.evaluate(x_val, y2_val_encoded, verbose=0)
print(accuracy_SS)